//
//  AddUtilitiesViewController.swift
//  NaturalDisaster
//
...
class AddUtilitiesViewController: UIViewController {
    @IBOutlet weak var nameTextField: UITextField!
    @IBOutlet weak var datePicker: UIDatePicker!
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "save" {
            let items = CoreDataHelper.retrieveUtilities()
            var listOfUtilities = [String]()
            for item in items {
                listOfUtilities.append(item.name!)
            }
...
//            } else
//                if isEdit == false {
//                    if listOfUtilities.contains(nameTextField.text!) {
...
//                        CoreDataHelper.saveUtilities()
//                    }
                
                } else {
                    
                    let item = CoreDataHelper.newUtilities()
                    if item.name != nil {
