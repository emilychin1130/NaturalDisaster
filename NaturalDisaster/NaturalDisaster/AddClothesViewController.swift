...
class AddClothesViewController: UIViewController {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "save" {
            let items = CoreDataHelper.retrieveClothes()
            var listOfClothes = [String]()
            for item in items {
                listOfClothes.append(item.name!)
            }
...
//            } else
//                if isEdit == false {
//                    if listOfClothes.contains(nameTextField.text!) {
...
//                        CoreDataHelper.saveClothes()
//                    }
//                    
                } else {
                    
                    let item = CoreDataHelper.newClothes()
                    if item.name != nil {
